...
if(isset($_SESSION['viewer_id'])) {
    ...
    $sql = "SELECT * FROM tbl_Users WHERE User_Id = '$viewer_id'";
    $result = mysqli_query($con, $sql);
    $row = mysqli_fetch_assoc($result);
    $user_id = $row['User_Id'];
    $user_name = $row['User_Name'];
} else {
    // Handle the case when viewer_id is not set in the session
    // Redirect or display an error message
    exit("Viewer ID not set in the session");
}
?>
<!DOCTYPE html>
<html lang="en">
...
if (isset($_GET['viewid'])) {
    ...
    if (isset($_POST['save'])) {
        $checkSaveQuery = "SELECT * FROM tbl_saves WHERE Recipe_Id = '$recipeid' AND User_Id = '$user_id'";
        $checkSaveResult = mysqli_query($con, $checkSaveQuery);

        if (mysqli_num_rows($checkSaveResult) == 0) {
            $insertSaveQuery = "INSERT INTO tbl_saves (Recipe_Id, User_Id) VALUES ('$recipeid', '$user_id')";
            mysqli_query($con, $insertSaveQuery);
    ...
    function hasSaved($recipeid, $user_id)
    {
        global $con;
        $checkSaveQuery = "SELECT * FROM tbl_saves WHERE Recipe_Id = '$recipeid' AND User_Id = '$user_id'";
        $checkSaveResult = mysqli_query($con, $checkSaveQuery);
        return mysqli_num_rows($checkSaveResult) > 0;
    }
    ...
    if (isset($_POST['user_id'], $_POST['user_name'], $_POST['rating_value'], $_POST['comments'])) {
  
        $stmt = $con->prepare('SELECT COUNT(*) FROM tbl_ratings WHERE Recipe_Id = ? AND User_Id = ?');
$stmt->bind_param('ii', $_GET['viewid'], $_POST['user_id']);
$stmt->execute();
...
if ($count > 0) {
    ...
    <script>
        Swal.fire({
            title: 'Error!',
            text: 'You have already submitted a review for this recipe!',
            icon: 'error'
        });
    </script>
    <?php
} else {
        $stmt = $con->prepare('INSERT INTO tbl_ratings (Recipe_Id, User_Id, User_Name, Rating_Value, Comments, Placed_On) VALUES (?, ?, ?, ?, ?, NOW())');
        $stmt->bind_param('iisss', $_GET['viewid'], $_POST['user_id'], $_POST['user_name'], $_POST['rating_value'], $_POST['comments']);
        $stmt->execute();
        ?>
    <script>
        Swal.fire({
  ...
  $reviews_info = $stmt->get_result()->fetch_assoc();
                                        ...
                                        <center>
                                            <div class="flex">
                                                <div class="inputBox">
                                                    ...
                                                    <?php } ?>
                                                    <form action="" method="POST" enctype="multipart/form-data" onsubmit="return handleSave()">
                                                        <button type="submit" name="save" class="dish-add-btn <?php echo hasSaved($recipeid, $user_id) ? 'saved' : ''; ?>" style="width:150px;">
                                                            <i class="uil uil-bookmark"></i>
                                                            <?php echo hasSaved($recipeid, $user_id) ? 'Recipe Saved' : 'Save Recipe'; ?>
                                                        </button>
                                                    </form>
    ...
    const writeReviewBtn = document.querySelector('.write_review_btn');
    const writeReviewDiv = document.querySelector('.write_review');
    ...
    writeReviewBtn.addEventListener('click', function (event) {
        ...
        writeReviewDiv.style.display = writeReviewDiv.style.display === 'none' ? 'block' : 'none';
    });
    ...
    sortSelect.addEventListener('change', function () {
        ...
        window.location.href = window.location.pathname + '?viewid=<?php echo $_GET['viewid']; ?>&sort_by=' + this.value;
                                            ...
                                            <div class="write_review" style="text-align:left;">
                                                <form method="POST">
                                                
                                                    <input type="hidden" name="user_id" value="<?php echo $user_id; ?>">
                                                    <div class="texbox">
                                                    <input type="text" name="user_name" value="<?php echo $user_name; ?>" style="width:50%; background-color:white; border-color:black;">
                                                    </div>
                                                    <input type="hidden" name="recipe_id" value="<?php echo $recipeid; ?>">
                                                    <div class="texbox">
                                                    <input name="rating_value" type="number" min="1" max="5" placeholder="Rating (1-5)" required style="width:50%; background-color:white; border-color:black;">
                                                    </div>
                                                    <textarea name="comments" placeholder="Write your review here..." cols="48" rows="3"required></textarea>
                                                    <br><br>
                                                    <td colspan="2" align="left"><input type="submit" class="btn" name="submit" value="Submit Review" style="width:30%; margin-left:10%;"></td>
